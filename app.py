...
def index():
    ...
    # greeterResponse = requests.get(greeterUrl, headers=headers)
    status, greeting = getGreeting(greeterUrl, headers)
...
def getGreeting(greeterUrl, headers):
    try:
        res = requests.get(greeterUrl, headers=headers, timeout=3.0)
    except:
        res = None
...
def getName(nameserviceUrl, headers):
    try:
        res = requests.get(nameserviceUrl, headers=headers, timeout=3.0)
    except:
        res = None
    if res and res.status_code == 200:
        return 200, res.text
        ...
        status = res.status_code if res is not None and res.status_code else 500
        return status, 'Sorry, Names not available.'
